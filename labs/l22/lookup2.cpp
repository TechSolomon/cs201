...
// Solomon Himelbloom
// 2 November 2020
// Sequential search with pair example for CS 201.
...
pair<bool, size_t>
    sequentialSearch(const vector<pair<int, string>> kvpairs, int thekey) {
        for (size_t i = 0; i < int(kvpairs.size()); ++i) {
            if (kvpairs[i].first == thekey) {
                return pair<bool, size_t>(true, i);
            }
        }
        return pair<bool, size_t>(false, kvpairs.size());
}
